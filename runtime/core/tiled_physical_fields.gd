class_name PGTiledPhysicalFields
extends RefCounted
...
static func terrain_height_m(cell: Vector2i, dimensions: Vector2i,
	...
	var c := PGPlanetGridContract.wrapped_global_cell(cell, dimensions)
	...
	var continent := PGAbsoluteFieldSampler.smooth_noise(c, dimensions, seed, continent_scale, 10)
	var macro := PGAbsoluteFieldSampler.smooth_noise(c, dimensions, seed, macro_scale, 11)
	var detail := PGAbsoluteFieldSampler.smooth_noise(c, dimensions, seed, ridge_scale, 12)
	var fine := PGAbsoluteFieldSampler.smooth_noise(c, dimensions, seed, detail_scale, 13)
	var ridge := pow(1.0 - abs(detail * 2.0 - 1.0), 3.0)
	var uplift_noise := PGAbsoluteFieldSampler.smooth_noise(c, dimensions, seed, macro_scale, 14)
	...
	match planet_type:
		2: # volcanic
			height += pow(PGAbsoluteFieldSampler.smooth_noise(c, dimensions, seed, ridge_scale, 21), 5.0) * 1800.0
		3: # airless
...
static func climate_at(cell: Vector2i, dimensions: Vector2i, seed: int,
	...
	var c := PGPlanetGridContract.wrapped_global_cell(cell, dimensions)
	var world := PGPlanetGridContract.global_cell_to_world(c, dimensions)
	var latitude_factor = abs(sin(world.y))
	var broad_scale := maxi(dimensions.x / 30, 8)
	var moisture := PGAbsoluteFieldSampler.smooth_noise(c, dimensions, seed, broad_scale, 31)
	var thermal := PGAbsoluteFieldSampler.smooth_noise(c, dimensions, seed, broad_scale * 2, 32)
...
static func plate_id(cell: Vector2i, dimensions: Vector2i, seed: int) -> int:
	...
	return PGAbsoluteFieldSampler.hash_cell(coarse, seed, 44) & 0x7FFFFFFF
