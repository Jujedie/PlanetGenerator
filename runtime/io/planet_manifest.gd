...
static func build(generation_params: Dictionary, exported_files: Dictionary, output_dir: String = "") -> Dictionary:
	...
	var dimensions: Vector2i = generation_params.get(
		"global_dimensions",
		generation_params.get("resolution", PGPlanetGridContract.logical_dimensions(radius_km))
	)
	var tile_size := int(generation_params.get("tile_size", PGPlanetGridContract.DEFAULT_TILE_SIZE))
	...
	return {
		...
		"grid": PGPlanetGridContract.contract_dictionary(radius_km, dimensions, tile_size),
		...
		"layer_formats": {
			...
			"runtime_query_data": "runtime_data/runtime_data_manifest.json + binary scalar/categorical layers",
		},
		"layers": layers,
	}
