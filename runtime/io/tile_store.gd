...
func has_complete_tile(layer: String, lod: int, tile: Vector2i) -> bool:
	...
	if not parsed is Dictionary:
		return false
	return str(parsed.get("sha256", "")) == PGFileChecksumCache.sha256(payload)
...
func write_tile(layer: String, lod: int, tile: Vector2i, payload: PackedByteArray,
	...
	file.close()
	var checksum := PGFileChecksumCache.sha256(temp_path)
	...
	if FileAccess.file_exists(final_path):
		PGFileChecksumCache.invalidate(final_path)
	...
	PGFileChecksumCache.invalidate(temp_path)
	PGFileChecksumCache.remember(final_path, checksum)
