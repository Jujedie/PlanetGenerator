class_name PGCartographicRenderer
extends RefCounted
...
static func render_full_map(geo_data: PackedByteArray, water_data: PackedByteArray,
		...
		sea_level: float, palette: PGCartographicPalette,
	...
	for marker_value in options.get("markers", []):
		...
		var cell := PGPlanetGridContract.world_to_global_cell(lon, lat, dimensions)
...
static func render_grid_overlay(dimensions: Vector2i, palette: PGCartographicPalette,
		options: Dictionary = {}) -> Dictionary:
	var pixel_count := dimensions.x * dimensions.y
	var output := PackedByteArray()
	output.resize(pixel_count * 4)
...
static func _base_color(height_m: float, water_type: int,
		palette: PGCartographicPalette, sea_level: float) -> Color:
	if water_type == 1:
...
static func _hillshade(geo_data: PackedByteArray, x: int, y: int,
	...
	var lon_lat := PGPlanetGridContract.global_cell_to_world(Vector2i(x, y), dimensions)
	...
	var lat0 := PGPlanetGridContract.global_cell_to_world(Vector2i(x, y0), dimensions).y
	var lat1 := PGPlanetGridContract.global_cell_to_world(Vector2i(x, y1), dimensions).y
...
static func _is_coordinate_grid(x: int, y: int, dimensions: Vector2i, view: String) -> bool:
	...
	var world := PGPlanetGridContract.global_cell_to_world(Vector2i(x, y), dimensions)
	var right := PGPlanetGridContract.global_cell_to_world(Vector2i((x + 1) % dimensions.x, y), dimensions)
	var down := PGPlanetGridContract.global_cell_to_world(Vector2i(x, mini(y + 1, dimensions.y - 1)), dimensions)
...
static func _modulate_biome(color: Color, biome_id: int, strength: float) -> Color:
	if strength <= 0.0 or biome_id == 0xFFFFFFFF:
		return color
	var hash := PGAbsoluteFieldSampler.hash_u32(biome_id * 2654435761)
