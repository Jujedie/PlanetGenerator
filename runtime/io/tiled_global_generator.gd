class_name PGTiledGlobalGenerator
extends RefCounted
...
var tile_size := PGPlanetGridContract.DEFAULT_TILE_SIZE
var hard_vram_budget_bytes := HARD_VRAM_BUDGET_BYTES
var preferred_vram_budget_bytes := PREFERRED_VRAM_BUDGET_BYTES
var cancel_token := PGGenerationCancelToken.new()
var last_report: Dictionary = {}

func _init(global_dimensions: Vector2i,
		configured_tile_size: int = PGPlanetGridContract.DEFAULT_TILE_SIZE) -> void:
	dimensions = global_dimensions
	tile_size = maxi(configured_tile_size, 64)

func build_tile_plan(halo: int = 0) -> Array:
	var plan: Array = []
	var grid := PGPlanetGridContract.tile_grid_dimensions(dimensions, tile_size)
	for ty in range(grid.y):
		for tx in range(grid.x):
			var tile := Vector2i(tx, ty)
			var geometry := PGPlanetGridContract.tile_rect_with_halo(tile, dimensions, halo, tile_size)
			plan.append({
				"tile": tile,
				"core": geometry["core"],
...
func run_phase(phase_name: String, output_dir: String, halo: int,
	...
	var store := PGPlanetTileStore.new(output_dir)
...
static func should_use_tiled(global_dimensions: Vector2i,
	...
	# production PGPlanetGeneratorBackend no longer enables that backend automatically.
...
static func absolute_cell_from_sample(descriptor: Dictionary,
	...
	return PGPlanetGridContract.wrapped_global_cell(origin + local_sample, dimensions_value)
