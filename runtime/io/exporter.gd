extends RefCounted
class_name PGPlanetExporter

## ============================================================================
## PLANET EXPORTER - GPU Texture to PNG with PGPlanetData.gd Color Palettes
## ============================================================================
...
func export_maps(gpu : PGGPUContext, output_dir: String, generation_params: Dictionary) -> Dictionary:
	"""
	Export all map types from GPU textures to PNG files
	Each individual export function handles its own threading internally
	
	Args:
		gpu: PGGPUContext with texture RIDs
	...
	# Récupérer l'instance PGGPUContext
	var gpu_context = gpu
	if not gpu_context:
		push_error("[Exporter] PGGPUContext not available!")
		return {}
	...
	var planet_type = int(params.get("planet_type", PGPlanetData.TYPE_TERRAN))
	...
	if planet_type == PGPlanetData.TYPE_GAZEUZE:
		print("[Exporter] 🪐 Export gazeuse - carte atmosphérique finale uniquement")
		# Gas giants have no queryable solid surface in the current core. Remove a
		# stale terrestrial runtime-data directory when an exact output path is reused.
		PGRuntimeDataWriter.remove_existing(output_dir)
		var exported_files = {}
		
		# Export final map only when requested by the active export policy.
		...
		if bool(params.get("run_integrity_checks", true)):
			var integrity_report := PGPlanetIntegrityChecker.run(gpu, params, exported_files, _readback_cache)
			var integrity_path := PGPlanetIntegrityChecker.save_report(output_dir, integrity_report)
			if not integrity_path.is_empty():
				exported_files["integrity_report"] = integrity_path
		exported_files = PGExportCatalog.finalize_outputs(output_dir, exported_files, params)
		var manifest_path := PGPlanetManifest.save(output_dir, params, exported_files)
		if not manifest_path.is_empty():
			exported_files["manifest"] = manifest_path
		var project_path := PGPlanetProject.save(output_dir, params, exported_files)
	...
	# PGGPUContext.prepare_for_export() deliberately frees every simulation shader
	...
	# PGGPUOrchestrator initialization. Otherwise all GPU export helpers silently
	...
	if bool(_export_stage_plan.get("topography", true)):
		export_shader_specs.append(["res://addons/planet_generator/runtime/shaders/compute/export/export_topography.glsl", "export_topography"])
	if bool(_export_stage_plan.get("final_map", true)):
		export_shader_specs.append(["res://addons/planet_generator/runtime/shaders/compute/export/export_final_map.glsl", "export_final_map"])
	if bool(_export_stage_plan.get("administration", true)):
		export_shader_specs.append(["res://addons/planet_generator/runtime/shaders/compute/export/export_id_colorize.glsl", "export_id_colorize"])
		export_shader_specs.append(["res://addons/planet_generator/runtime/shaders/compute/export/export_admin_extract.glsl", "export_admin_extract"])
		export_shader_specs.append(["res://addons/planet_generator/runtime/shaders/compute/export/export_admin_hierarchy.glsl", "export_admin_hierarchy"])
	if bool(_export_stage_plan.get("resources", true)):
		export_shader_specs.append(["res://addons/planet_generator/runtime/shaders/compute/export/export_resource_map.glsl", "export_resource_map"])
	...
	# Geo is authoritative. Presentation topography is optional, while addon
	# runtime-data export may still read geo later even when no topographic PNG is requested.
	if bool(_export_stage_plan.get("topography", true)):
	...
	elif planet_type in [PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE] and bool(
		_export_stage_plan.get("water", false)
	):
		# Dry worlds still keep the explicit transparent water-map contract.
	...
	if bool(_export_stage_plan.get("climate", true)):
		if planet_type in [PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE]:
	...
	# === EXPORT EAUX / RIVIÈRES ===
	stage_started = Time.get_ticks_usec()
	if planet_type not in [PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE]:
	...
	# Keep river_biome_id until the addon runtime-data snapshot. For non-resource
	# exports, retain river_flux in cache so integrity can reuse the same bytes.
	if bool(_export_stage_plan.get("resources", true)):
	...
	if bool(_export_stage_plan.get("administration", true)):
		var administrative_result := _export_administrative_maps_hybrid(
			gpu, output_dir, planet_type not in [PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE]
		)
	...
	# === RUNTIME QUERY DATA (Addon API) ===
	# This is authoritative game data and is deliberately independent from the
	# presentation-map export preset.
	stage_started = Time.get_ticks_usec()
	if bool(params.get("export_runtime_query_data", true)):
		var runtime_manifest := _export_runtime_query_data(gpu, output_dir, width, height)
		if not runtime_manifest.is_empty():
			exported_files["runtime_data_manifest"] = runtime_manifest
	else:
		PGRuntimeDataWriter.remove_existing(output_dir)
	_record_stage("runtime_query_data", stage_started)
	...
	# Complete/development resource exports can be large: free presentation
	# readback cache first. Minimal/standard exports keep cached integrity inputs.
	if bool(_export_stage_plan.get("resources", true)):
		_clear_readback_cache()
	if _abort_export_if_cancelled(export_started_usec):
		return exported_files
...
func _assign_administrative_colors(group_ids: Array) -> Dictionary:
	...
	if _admin_color_cursor + count > PGHierarchyBuilder.ADMIN_COLOR_CAPACITY:
		push_error(
			"Administrative export needs %d unique colors, but RGBA8 can represent only %d reserved-safe colors"
			% [_admin_color_cursor + count, PGHierarchyBuilder.ADMIN_COLOR_CAPACITY]
		)
		return {}
	var colors := PGHierarchyBuilder.assign_colors(group_ids, _admin_color_cursor)
...
func _output_requested(output_dir: String, key: String, filename: String) -> bool:
	var planet_type := int(params.get("planet_type", PGPlanetData.TYPE_TERRAN))
	# Gas giants only expose their atmospheric final render.
	if planet_type == PGPlanetData.TYPE_GAZEUZE and key != "final_map":
		return false
	# Dry worlds keep an explicit transparent water map, but never export river,
	# cloud/ice-cap, or maritime administrative presentation products.
	if planet_type in [PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE] and key in [
	...
	if key == "grid_overlay" and not bool(params.get("export_grid_overlay", true)):
		return false
	return PGExportCatalog.should_keep(key, params, output_dir.path_join(filename))
...
func _resource_outputs_requested(output_dir: String) -> bool:
	...
	if PGExportCatalog.should_keep(
		"petrole_map", params, resource_dir.path_join("petrole_map.png")
	):
		return true
	for resource_name in RESOURCE_NAMES:
		var key := str(resource_name) + "_map"
		if PGExportCatalog.should_keep(key, params, resource_dir.path_join(key + ".png")):
			return true
	return false
...
func _prune_unrequested_known_outputs(output_dir: String) -> void:
	# Exact-directory jobs can reuse a previous planet folder. Remove only known
	# Planet Generator PNGs that the current preset no longer requests.
	var known_outputs := [
...
func _export_runtime_query_data(gpu: PGGPUContext, output_dir: String,
		width: int, height: int) -> String:
	var writer := PGRuntimeDataWriter.new()
	if not writer.begin(output_dir, Vector2i(width, height), params):
		return ""

	# Height is the authoritative geo.r value. The reader subtracts the
	# configured sea level for get_height_at(), while get_surface_elevation_at()
	# exposes the unshifted value.
	if gpu.textures.has("geo") and gpu.textures["geo"].is_valid():
		writer.write_height_from_geo(_read_texture(gpu, "geo"))

	# Climate RGBA32F stores temperature in R and normalized
	# humidity/precipitation in G. Only those two public channels are persisted.
	if gpu.textures.has("climate") and gpu.textures["climate"].is_valid():
		var climate_data := _read_texture(gpu, "climate")
		if not writer.write_climate_from_rgba32f(climate_data):
			push_error("[Exporter] Runtime query-data climate export failed; temperature_c/precipitation will be unavailable")
		climate_data = PackedByteArray()
	else:
		push_error("[Exporter] Authoritative climate texture is unavailable during runtime query-data export")

	if gpu.textures.has("biome_id") and gpu.textures["biome_id"].is_valid():
		writer.write_u32_layer(
			"biome_id", "biome_id.r32ui", _read_texture(gpu, "biome_id"),
			"R32UI ground biome id; index into manifest.biomes"
		)
	if gpu.textures.has("river_biome_id") and gpu.textures["river_biome_id"].is_valid():
		writer.write_u32_layer(
			"river_biome_id", "river_biome_id.r32ui", _read_texture(gpu, "river_biome_id"),
			"R32UI river biome id; 0xFFFFFFFF means no river; index into manifest.river_biomes"
		)
	if gpu.textures.has("water_mask") and gpu.textures["water_mask"].is_valid():
		writer.write_u8_layer(
			"water_type", "water_type.r8ui", _read_texture(gpu, "water_mask"),
			"R8UI 0=land 1=saltwater 2=freshwater"
		)
	if gpu.textures.has("region_map") and gpu.textures["region_map"].is_valid():
		writer.write_u32_layer(
			"region_id", "region_id.r32ui", _read_texture(gpu, "region_map"),
			"R32UI base land administrative region id; 0xFFFFFFFF=no-data"
		)
	if gpu.textures.has("ocean_region_map") and gpu.textures["ocean_region_map"].is_valid():
		writer.write_u32_layer(
			"ocean_region_id", "ocean_region_id.r32ui", _read_texture(gpu, "ocean_region_map"),
			"R32UI base ocean administrative region id; 0xFFFFFFFF=no-data"
		)

	var manifest_path := writer.finish()
	if not manifest_path.is_empty():
		print("[Exporter] ✅ Runtime query data: ", manifest_path)
	return manifest_path


func _read_texture(gpu: PGGPUContext, texture_name: String) -> PackedByteArray:
...
func _read_texture_rid(gpu: PGGPUContext, texture_rid: RID,
...
func _record_readback_metrics(gpu: PGGPUContext, texture_name: String,
	...
	if gpu.textures.has(texture_name):
		...
		if texture_format.format == PGGPUContext.FORMAT_STATE:
...
func _save_png(image: Image, filepath: String) -> Error:
	...
	PGFileChecksumCache.invalidate(filepath)
...
func _consume_png_result(filepath: String, worker_result: Dictionary, queued_usec: int = 0) -> void:
	...
	if error == OK:
		...
		if not checksum.is_empty():
			PGFileChecksumCache.remember(filepath, checksum)
	else:
...
func _create_export_rgba8_texture(gpu: PGGPUContext, width: int, height: int) -> RID:
	var format := RDTextureFormat.new()
	format.width = width
	format.height = height
	format.format = PGGPUContext.FORMAT_RGBA8
	format.usage_bits = (
...
func _build_topography_palette_buffer(
	...
	var color_thresholds: Array = PGPlanetData.COULEURS_ELEVATIONS.keys()
	var grey_thresholds: Array = PGPlanetData.COULEURS_ELEVATIONS_GREY.keys()
	color_thresholds.sort()
	grey_thresholds.sort()
	var bytes := PackedByteArray()
	# One trailing vec4 carries the dry-world relief range without changing the
	# established 32-byte push-constant ABI.
	bytes.resize((color_thresholds.size() + grey_thresholds.size() + 1) * 16)
	var cursor := 0
	for threshold in color_thresholds:
		var color: Color = PGPlanetData.COULEURS_ELEVATIONS[threshold]
	...
	for threshold in grey_thresholds:
		var color: Color = PGPlanetData.COULEURS_ELEVATIONS_GREY[threshold]
		bytes.encode_float(cursor, float(threshold))
		bytes.encode_float(cursor + 4, color.r)
		bytes.encode_float(cursor + 8, color.g)
		bytes.encode_float(cursor + 12, color.b)
...
func _render_topography_gpu(gpu: PGGPUContext, width: int, height: int,
	...
	var color_count := PGPlanetData.COULEURS_ELEVATIONS.size()
	var grey_count := PGPlanetData.COULEURS_ELEVATIONS_GREY.size()
...
func _render_id_color_map_gpu(gpu: PGGPUContext, texture_name: String,
...
func _release_admin_extract_resources(gpu: PGGPUContext, resources: Dictionary) -> void:
...
func _dispatch_admin_extract_once(gpu: PGGPUContext, source_texture: String,
...
func _extract_admin_graph_gpu(gpu: PGGPUContext, source_texture: String,
...
func _build_admin_level_colors(hierarchy: Array) -> Array:
	...
	for level_index in range(3):
		...
		if mapping.is_empty():
			colors.append({})
			continue
		colors.append(_assign_administrative_colors(PGHierarchyBuilder._unique_values(mapping)))
	return colors
...
func _render_admin_family_gpu(gpu: PGGPUContext, source_texture: String,
...
func _export_administrative_maps_legacy(gpu: PGGPUContext, output_dir: String,
...
func _export_administrative_maps_hybrid(gpu: PGGPUContext, output_dir: String,
		include_sea: bool) -> Dictionary:
	...
	var land_hierarchy := PGHierarchyBuilder.build_land_from_graph(
	...
	if not sea_graph.is_empty():
		...
		sea_hierarchy = PGHierarchyBuilder.build_sea_from_graph(
...
func _create_resource_export_renderer(gpu: PGGPUContext, width: int, height: int) -> Dictionary:
...
func _render_resource_map_gpu(gpu: PGGPUContext, renderer: Dictionary,
...
func _destroy_resource_export_renderer(gpu: PGGPUContext, renderer: Dictionary) -> void:
	if renderer.has("set"):
...
## - Version colorée : utilise COULEURS_ELEVATIONS d'PGPlanetData.gd
## - Version grisée : utilise COULEURS_ELEVATIONS_GREY d'PGPlanetData.gd
...
func _export_topographie_maps(gpu: PGGPUContext, geo_data: PackedByteArray,
	...
	_waterless_elevation_range = Vector2(minimum_relative_elevation, maximum_relative_elevation)
...
func _build_topography_cpu_fallback(geo_values: PackedFloat32Array, width: int,
	...
	var has_water := int(params.get("planet_type", PGPlanetData.TYPE_TERRAN)) not in [
		PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE
	]
	var waterless_span := maxf(maximum_relative_elevation - minimum_relative_elevation, 1.0)
	for pixel_index in range(pixel_count):
		...
		var color: Color = PGPlanetData.getElevationColor(colored_elevation, false)
		var grey_color: Color = PGPlanetData.getElevationColor(elevation_int, true)
...
func _export_plates_map(plates_data: PackedByteArray, output_dir: String,
	...
	var want_plates := PGExportCatalog.should_keep("plaques_map", params, path_plates)
	var want_borders := PGExportCatalog.should_keep(
...
func _export_climate_maps_without_clouds(gpu: PGGPUContext, output_dir: String) -> Dictionary:
	...
	for tex_id in climate_textures.keys():
		var filename := str(climate_textures[tex_id])
		if not _output_requested(output_dir, str(tex_id), filename):
			continue
...
##
## @param gpu: Instance PGGPUContext avec les textures climat
## @param output_dir: Dossier de sortie
## @return Dictionary: Chemins des fichiers exportés
func _export_climate_maps_optimized(gpu: PGGPUContext, output_dir: String) -> Dictionary:
...
##
## @param gpu: Instance PGGPUContext avec la texture region_colored
## @param output_dir: Dossier de sortie
## @return Dictionary: Chemin du fichier exporté
func _export_region_map(gpu: PGGPUContext, output_dir: String,
	...
	var merge_map := PGHierarchyBuilder.compute_merge_map(data, width, height)
...
##
## @param gpu: Instance PGGPUContext avec la texture ocean_region_map
## @param output_dir: Dossier de sortie
## @return Dictionary: Chemin du fichier exporté
func _export_ocean_region_map(gpu: PGGPUContext, output_dir: String,
		_optimised_region_generation: bool = true) -> Dictionary:
...
func _export_biome_map(gpu: PGGPUContext, output_dir: String) -> Dictionary:
	...
	print("[Exporter] ✅ PGBiome export complete")
	return result
...
##
## @param gpu: Instance PGGPUContext avec les textures ressources
## @param output_dir: Dossier de sortie principal
## @param width: Largeur de l'image
## @param height: Hauteur de l'image
## @return Dictionary: Chemins des fichiers exportés
func _export_resources_maps(gpu: PGGPUContext, output_dir: String, width: int,
	...
	# Do not materialize files that PGExportCatalog will immediately delete. This is
	...
	if PGExportCatalog.should_keep("petrole_map", params, petrole_path):
	...
	for name in RESOURCE_NAMES:
		...
		if PGExportCatalog.should_keep(key, params, path):
	...
	for resource in PGPlanetData.RESSOURCES:
...
func _export_resources_cpu_fallback(gpu: PGGPUContext, resources_dir: String,
...
##
## @param gpu: Instance PGGPUContext avec les textures
## @param output_dir: Dossier de sortie
## @param width: Largeur de l'image
## @param height: Hauteur de l'image
## @return Dictionary: Chemins des fichiers exportés
func _export_empty_water_map(output_dir: String, width: int, height: int) -> Dictionary:
	# Airless/sterile worlds have no liquid-water phase. Keep the historical
	# explicit water output as a transparent image when the active preset asks for it.
	var pixels := PackedByteArray()
	pixels.resize(width * height * 4)
...
func _export_water_classification(gpu: PGGPUContext, output_dir: String, width: int, height: int) -> Dictionary:
...
func _export_river_maps_paired(gpu: PGGPUContext, output_dir: String,
		width: int, height: int) -> Dictionary:
	var result: Dictionary = {}
	...
	var atmosphere_type := int(params.get("planet_type", PGPlanetData.TYPE_TERRAN))
	var river_biomes_list: Array = PGPlanetData.get_river_biomes_for_gpu(atmosphere_type)
	...
	if want_river:
		...
		var river_image := Image.create_from_data(width, height, false, Image.FORMAT_RGBA8, river_pixels)
		if _save_png(river_image, river_path) == OK:
			result["river_map"] = river_path
	if want_type:
		var type_path := output_dir.path_join("river_type_map.png")
		var type_image := Image.create_from_data(width, height, false, Image.FORMAT_RGBA8, type_pixels)
		if _save_png(type_image, type_path) == OK:
			result["river_type_map"] = type_path
	return result


func _export_requested_river_fallbacks(gpu: PGGPUContext, output_dir: String,
...
func _export_river_map(gpu: PGGPUContext, output_dir: String, width: int, height: int) -> Dictionary:
...
func _export_river_type_map(gpu: PGGPUContext, output_dir: String, width: int, height: int) -> Dictionary:
...
## - PGBiome (couleur de base végétation)
...
##
## @param gpu: Instance PGGPUContext avec la texture final_map
## @param output_dir: Dossier de sortie
## @return Dictionary: Chemin du fichier exporté
func _export_cartographic_map(gpu: PGGPUContext, output_dir: String) -> Dictionary:
	...
	if water_data.size() != pixel_count:
		...
		return {}
	var palette_path := str(params.get("cartography_palette_path", PGCartographicPalette.DEFAULT_PATH))
	var palette := PGCartographicPalette.load_palette(palette_path)
	var rendered := PGCartographicRenderer.render_full_map(
		geo_data, water_data, biome_data, dimensions,
		float(params.get("planet_radius", 150.0)),
		float(params.get("sea_level", 0.0)), palette, {
			"view": str(params.get("cartography_view", PGCartographicRenderer.VIEW_PLANET)),
			"markers": params.get("cartography_markers", []),
			"waterless_surface": int(params.get("planet_type", PGPlanetData.TYPE_TERRAN)) in [
				PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE,
			],
...
func _export_grid_overlay(gpu: PGGPUContext, output_dir: String) -> Dictionary:
	...
	var palette_path := str(params.get("cartography_palette_path", PGCartographicPalette.DEFAULT_PATH))
	var palette := PGCartographicPalette.load_palette(palette_path)
	var rendered := PGCartographicRenderer.render_grid_overlay(dimensions, palette, {
		"view": str(params.get("cartography_view", PGCartographicRenderer.VIEW_PLANET)),
...
func _final_map_darkening_factor() -> float:
	...
	if planet_type == PGPlanetData.TYPE_TOXIC:
		return 0.92
	if planet_type == PGPlanetData.TYPE_VOLCANIC:
		return 0.93
	if planet_type == PGPlanetData.TYPE_DEAD:
		return 0.82
	return WATER_DARKENING_FACTOR


func _render_final_export_gpu(gpu: PGGPUContext, width: int, height: int,
...
func _build_final_export_cpu_fallback(gpu: PGGPUContext, width: int, height: int,
		darkening_factor: float) -> Image:
	var expected_size := width * height * 4
...
func _export_final_map(gpu: PGGPUContext, output_dir: String) -> Dictionary:
...
func _export_hierarchy_maps(gpu: PGGPUContext, output_dir: String) -> Dictionary:
	...
	var merge_land := PGHierarchyBuilder.compute_merge_map(land_data, width, height)
	var merge_sea: Dictionary = {}
	if not sea_data.is_empty():
		merge_sea = PGHierarchyBuilder.compute_merge_map(sea_data, width, height)
	
	# ─── Construction des hiérarchies (BFS) ──────────────────────────────────
	print("  Hiérarchie terrestre :")
	var land := PGHierarchyBuilder.build_land(land_data, width, height, merge_land, params)
	# land = [dept→région, dept→pays, dept→continent]
	
	var sea: Array = [{}, {}, {}]
	if not sea_data.is_empty():
		print("  Hiérarchie maritime :")
		sea = PGHierarchyBuilder.build_sea(
	...
	for entry in exports:
		...
		if d2g.is_empty():
			...
			continue
		var group_ids := PGHierarchyBuilder._unique_values(d2g)
...
func _recover_missing_saltwater_mask(gpu: PGGPUContext,
