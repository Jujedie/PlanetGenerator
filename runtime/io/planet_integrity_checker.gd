class_name PGPlanetIntegrityChecker
extends RefCounted
...
static func run(gpu: PGGPUContext, generation_params: Dictionary,
...
static func _resolve_dimensions(gpu: PGGPUContext, params: Dictionary,
...
static func _read_texture(gpu: PGGPUContext, texture_name: String,
...
static func _check_exports(checks: Array[Dictionary], metrics: Dictionary,
	...
	# Validate the keys actually returned by PGPlanetExporter, not obsolete aliases.
	# The selected export preset is authoritative: integrity must not fail simply
	# because a Minimal/Custom policy intentionally skipped a presentation map.
	...
	for candidate in required_candidates:
		if PGExportCatalog.should_keep(str(candidate[0]), generation_params, str(candidate[1])):
...
static func _check_river_export_consistency(checks: Array[Dictionary], metrics: Dictionary,
	...
	if planet_type in [3, 5, 6]:
		...
		return
	var wants_river := PGExportCatalog.should_keep(
		"river_map", generation_params, "river_map.png"
	)
	var wants_type := PGExportCatalog.should_keep(
