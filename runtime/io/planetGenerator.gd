...
class_name PGPlanetGeneratorBackend
...
var gpu_orchestrator    : PGGPUOrchestrator = null
var use_gpu_acceleration: bool            = true
var use_tiled_global_generation: bool     = false
var tiled_pipeline: PGTiledGlobalSimulationPipeline = null
...
# Generation parameters (compiled from a generation spec)
var generation_params: Dictionary = {}
var _cleaned_up: bool = false
var _generation_request_id: int = 0

## Constructeur de la classe PGPlanetGeneratorBackend.
##
...
func _init(nom_param: String, generation_param : Dictionary, cheminSauvegarde_param: String = "user://planet_generator/generated"):

	"""
	PGPlanetGeneratorBackend constructor
...
##
## Instancie le [PGGPUContext] (si nécessaire) et configure le [PGGPUOrchestrator]
...
func _init_gpu_system() -> void:
	...
	var monolithic_supported := PGTiledGlobalGenerator.fits_monolithic_envelope(
		global_dimensions
	)
	...
	use_tiled_global_generation = (
		...
		and PGTiledGlobalGenerator.PRODUCTION_TILED_ENABLED
	)
	...
	if planet_type != 6 and monolithic_supported:
		generation_params["large_monolithic_lifecycle"] = (
			PGTiledGlobalGenerator.exceeds_preferred_monolithic_budget(global_dimensions)
		)
		generation_params["tiled_global_generation"] = false
		print(
			"[PGPlanetGeneratorBackend] Authoritative monolithic GPU backend configured: ",
			global_dimensions,
			" (large lifecycle=",
			generation_params["large_monolithic_lifecycle"],
			")"
		...
		print("[PGPlanetGeneratorBackend] Experimental tiled generation enabled: ", global_dimensions)
	elif planet_type == 6:
		use_tiled_global_generation = false
		print("[PGPlanetGeneratorBackend] Gas giant GPU backend configured: ", generation_params["resolution"])
	else:
		use_tiled_global_generation = false
		generation_params["tiled_global_generation"] = false
		push_error((
			"[PGPlanetGeneratorBackend] Resolution %s exceeds the safe monolithic GPU envelope. "
...
func generate_planet() -> bool:
	...
	if _cleaned_up or not use_gpu_acceleration:
		print("[PGPlanetGeneratorBackend] Cancelling generation: GPU acceleration not available")
		return false
	_generation_request_id += 1
	_cached_display_maps.clear()
	_cached_export_files.clear()
	if use_tiled_global_generation:
		print("[PGPlanetGeneratorBackend] Queuing tiled global generation on background GPU worker...")
		tiled_pipeline = PGTiledGlobalSimulationPipeline.new(generation_params, _tiled_output_root)
		tiled_pipeline.phase_started.connect(_on_tiled_phase_started)
		tiled_pipeline.tile_progress.connect(_on_tiled_tile_progress)
		if not PGGPUGenerationWorker.submit(_run_tiled_generation_worker.bind(_generation_request_id)):
			push_error("[PGPlanetGeneratorBackend] Unable to queue tiled generation worker")
			tiled_pipeline = null
			return false
		return true
	if _monolithic_job_active:
		push_warning("[PGPlanetGeneratorBackend] Monolithic generation is already running")
		return false
	_cancel_mutex.lock()
	...
	print("[PGPlanetGeneratorBackend] Queuing monolithic GPU generation on background worker...")
	if not PGGPUGenerationWorker.submit(_run_monolithic_generation_worker.bind(_generation_request_id)):
		_monolithic_job_active = false
		push_error("[PGPlanetGeneratorBackend] Unable to queue background GPU generation")
		return false
	return true
...
func _complete_tiled_generation(request_id: int, report: Dictionary) -> void:
	...
	if bool(report.get("ok", false)):
		...
		print("[PGPlanetGeneratorBackend] Experimental tiled dataset complete: ", report.get("manifest", ""))
		...
		push_error("[PGPlanetGeneratorBackend] Tiled generation failed: %s" % report.get("reason", "unknown"))

func _on_monolithic_phase_started(phase: String, index: int, total: int) -> void:
	# Orchestrator signals are emitted on the GPU worker. Bounce caller-facing
...
func cancel_generation(reason: String = "user") -> void:
	...
	_cancel_mutex.unlock()
	if first_request:
		print("[PGPlanetGeneratorBackend] Cancellation requested by caller: ", reason)
	if tiled_pipeline != null:
...
func _run_monolithic_generation_worker(request_id: int) -> void:
	# This entire method executes on PGGPUGenerationWorker's persistent thread.
	...
	if bool(cancel_state.get("cancelled", false)):
		...
		return
	var display_maps: Array[String] = PGPlanetProject.display_maps_from_layers(exported_files)
	last_performance_report = orchestrator.last_performance_report.duplicate(true)
	...
	# keeping PGGPUContext's shared device alive for the next generation.
	orchestrator.cleanup()
	if gpu_orchestrator == orchestrator:
		gpu_orchestrator = null
...
func _complete_monolithic_generation(request_id: int, ok: bool, display_maps: Array, reason: String) -> void:
	...
	if ok:
		...
		push_error("[PGPlanetGeneratorBackend] Background generation failed: %s" % reason)
		emit_signal("generation_cancelled", reason)

func export_to_directory(output_dir: String) -> Dictionary:
	"""Return/copy completed exports without requiring live GPU resources."""
	print("[PGPlanetGeneratorBackend] Exporting to: ", output_dir)
	if use_tiled_global_generation and tiled_pipeline != null:
		...
		if output_dir.simplify_path() != cheminSauvegarde.simplify_path():
			if not tiled_pipeline.export_dataset(output_dir):
				push_warning("[PGPlanetGeneratorBackend] Tiled export skipped: dataset is incomplete")
				return {}
		last_exported_files = {"tiled_dataset": _tiled_output_root}
		...
		push_warning("[PGPlanetGeneratorBackend] Export skipped: generation resources are unavailable")
	print("[PGPlanetGeneratorBackend] Export complete")
...
func _copy_cached_exports(output_dir: String) -> void:
	...
	if not PGPlanetTileStore.copy_tree(_generation_output_root, output_dir):
		push_warning("[PGPlanetGeneratorBackend] Failed to copy generated project %s -> %s" % [
...
## et retourne les chemins. Conservé comme adaptateur interne pour les exports terminés.
##
...
## Prepare only this job's output directory. The addon never clears a shared
## shared temporary tree because that could delete files belonging to the host game.
func _prepare_output_directory() -> void:
	DirAccess.make_dir_recursive_absolute(_generation_output_root)
	_clear_directory_contents(_generation_output_root)
