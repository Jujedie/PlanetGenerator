class_name PGGlobalHydrologyContext
extends RefCounted
...
static func build(dimensions: Vector2i, params: Dictionary,
		tile_size: int = PGPlanetGridContract.DEFAULT_TILE_SIZE,
		max_macro_width: int = 512) -> PGGlobalHydrologyContext:
	var context := PGGlobalHydrologyContext.new()
...
static func build_from_height_tiles(dimensions: Vector2i, params: Dictionary,
		store: PGPlanetTileStore, tile_size: int = PGPlanetGridContract.DEFAULT_TILE_SIZE,
		max_macro_width: int = 512) -> PGGlobalHydrologyContext:
	var context := PGGlobalHydrologyContext.new()
	context.global_dimensions = dimensions
	context.stride = maxi(1, ceili(float(dimensions.x) / float(maxi(max_macro_width, 32))))
...
func _build_fields_from_store(params: Dictionary, store: PGPlanetTileStore, tile_size: int) -> void:
	...
	for y in range(macro_dimensions.y):
		for x in range(macro_dimensions.x):
			...
			var address := PGPlanetGridContract.cell_to_tile(cell, global_dimensions, tile_size)
			...
			var rect := PGPlanetGridContract.tile_rect(tile, global_dimensions, tile_size)
...
func _build_fields(params: Dictionary) -> void:
	...
	for y in range(macro_dimensions.y):
		for x in range(macro_dimensions.x):
			...
			elevation[index] = PGTiledPhysicalFields.terrain_height_m(
...
func _build_routes_and_flux(params: Dictionary) -> void:
	...
	for y in range(macro_dimensions.y):
		for x in range(macro_dimensions.x):
			...
			var climate := PGTiledPhysicalFields.climate_at(
...
func _build_tile_routes(tile_size: int) -> void:
	var grid := PGPlanetGridContract.tile_grid_dimensions(global_dimensions, tile_size)
	...
	tile_routes = PGGlobalDrainageRouter.route(outlets, grid)
