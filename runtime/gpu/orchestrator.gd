...
##
## @param gpu_context: Référence vers le gestionnaire de bas niveau [PGGPUContext].
...
func _init(gpu_context: PGGPUContext, res: Vector2i = Vector2i(128, 64), gen_params: Dictionary = {}) -> void:
	gpu = gpu_context
	resolution = res
	...
	# ✅ VALIDATION 1: PGGPUContext existe
	if not gpu:
		push_error("[Orchestrator] ❌ FATAL: PGGPUContext is null")
		return
	...
	if not rd:
		...
		push_error("  Le PGGPUContext n'a pas pu initialiser le GPU")
		return
...
##
## Charge les fichiers `.glsl` depuis le disque (the addon shader directory) et les compile en bytecode SPIR-V via le [PGGPUContext].
...
func _compile_all_shaders() -> bool:
	...
	var shaders_to_load = [
		...
		{"path": "res://addons/planet_generator/runtime/shaders/compute/topographie/base_elevation.glsl", "name": "base_elevation", "critical": true},
		# Shaders d'âge de croûte (JFA + Finalisation)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/topographie/crust_age_jfa.glsl", "name": "crust_age_jfa", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/topographie/crust_age_finalize.glsl", "name": "crust_age_finalize", "critical": false},
		# Shader de cratères (planètes sans atmosphère)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/topographie/cratering.glsl", "name": "cratering", "critical": false},
		# Shaders Érosion Hydraulique (Étape 2)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/erosion/erosion_rainfall.glsl", "name": "erosion_rainfall", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/erosion/erosion_flow.glsl", "name": "erosion_flow", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/erosion/erosion_sediment.glsl", "name": "erosion_sediment", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/erosion/erosion_flux_accumulation.glsl", "name": "erosion_flux_accumulation", "critical": false},
		# Shaders Atmosphère & Climat (Étape 3)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/atmosphere_climat/temperature.glsl", "name": "temperature", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/atmosphere_climat/precipitation.glsl", "name": "precipitation", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/atmosphere_climat/clouds.glsl", "name": "clouds", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/atmosphere_climat/ice_caps.glsl", "name": "ice_caps", "critical": false},
		# Shaders Ressources & Pétrole (Étape 5)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/ressources/petrole.glsl", "name": "petrole", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/ressources/resources.glsl", "name": "resources", "critical": false},
		# Shaders Classification des Eaux & Rivières (Étape 2.5)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/water/water_fill.glsl", "name": "water_fill", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/water/river_sources.glsl", "name": "river_sources", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/water/river_classify.glsl", "name": "river_classify", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/water/river_type_assign.glsl", "name": "river_type_assign", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/water/hydrology_water_color.glsl", "name": "hydrology_water_color", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/water/disabled_hydrology_clear.glsl", "name": "disabled_hydrology_clear", "critical": false},
		# Shaders Régions Administratives (Étape 4)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/region/region_seed_placement.glsl", "name": "region_seed_placement", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/region/region_edge_cost.glsl", "name": "region_edge_cost", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/region/region_growth.glsl", "name": "region_growth", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/region/region_cleanup.glsl", "name": "region_cleanup", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/region/region_finalize.glsl", "name": "region_finalize", "critical": false},
		# Shaders Régions Océaniques (Étape 4.5)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/ocean_region/ocean_region_seed_placement.glsl", "name": "ocean_region_seed_placement", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/ocean_region/ocean_region_edge_cost.glsl", "name": "ocean_region_edge_cost", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/ocean_region/ocean_region_growth.glsl", "name": "ocean_region_growth", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/ocean_region/ocean_region_cleanup.glsl", "name": "ocean_region_cleanup", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/ocean_region/ocean_region_finalize.glsl", "name": "ocean_region_finalize", "critical": false},
		# Shaders Biomes (Étape 4.1)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/biome/biome_classify.glsl", "name": "biome_classify", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/biome/biome_smooth.glsl", "name": "biome_smooth", "critical": false},
		# Shaders Final Map (Étape 6)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/final_map.glsl", "name": "final_map", "critical": false},
		# Shader Gas Giant Final Map (Type 6 - Gazeuse)
		{"path": "res://addons/planet_generator/runtime/shaders/compute/gas_giant_final.glsl", "name": "gas_giant_final", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/gas_giant/gas_giant_velocity_init.glsl", "name": "gas_giant_velocity_init", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/gas_giant/gas_giant_dye_init.glsl", "name": "gas_giant_dye_init", "critical": false},
		{"path": "res://addons/planet_generator/runtime/shaders/compute/gas_giant/gas_giant_advect.glsl", "name": "gas_giant_advect", "critical": false},
	]
...
func _init_textures():
	"""Valide les textures d'état créées par PGGPUContext et recrée les absentes."""
	...
	# PGGPUContext initialise déjà ces textures. L'ancienne version les recréait
...
func log_all_shader_rids():
	if not gpu or not gpu.shaders:
		...
		return
	print("[DEBUG] Liste des shader RIDs dans PGGPUContext :")
...
func run_simulation() -> void:
	...
	_cancel_reason = ""
	_phase_counter = 0
	_phase_total = 1 if int(generation_params.get("planet_type", 0)) == PGPlanetData.TYPE_GAZEUZE else 13
	...
	if atmosphere_type == PGPlanetData.TYPE_GAZEUZE:
...
func run_cratering_phase(params: Dictionary, w: int, h: int) -> void:
	...
	if atmosphere_type not in [PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_DEAD, PGPlanetData.TYPE_STERILE]:
...
func run_erosion_phase(params: Dictionary, w: int, h: int) -> void:
	...
	var atmosphere_type = int(params.get("planet_type", 0))
	if atmosphere_type in [PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE]:  # Sans atmosphère ou Stérile
...
func run_pre_erosion_climate_phase(params: Dictionary, w: int, h: int) -> void:
	var atmosphere_type = int(params.get("planet_type", 0))
	if atmosphere_type in [PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE]:
		return
...
func _dispatch_temperature(w: int, h: int, groups_x: int, groups_y: int, seed_val: int, avg_temperature: float, sea_level: float, cylinder_radius: float, atmosphere_type: int) -> void:
	...
	var palette_data: PackedByteArray = PGPlanetData.build_temperature_palette(atmosphere_type)
...
func _dispatch_precipitation(w: int, h: int, groups_x: int, groups_y: int, seed_val: int, avg_precipitation: float, cylinder_radius: float, atmosphere_type: int, sea_level: float = 0.0) -> void:
	...
	# === SET 2 : PALETTE DE COULEURS DYNAMIQUE (SSBO) ===
	var palette_data: PackedByteArray = PGPlanetData.build_precipitation_palette(atmosphere_type)
	var palette_ssbo: RID = rd.storage_buffer_create(palette_data.size(), palette_data)
	if not palette_ssbo.is_valid():
...
func run_water_phase(params: Dictionary, w: int, h: int) -> void:
	...
	if atmosphere_type in [PGPlanetData.TYPE_NO_ATMOS, PGPlanetData.TYPE_STERILE]:
		...
		var cleared_on_gpu := _dispatch_disabled_hydrology_clear(w, h, groups_x, groups_y)
	...
	var solver := PGHydrologySolver.new()
	...
	_administrative_water_mask_size = Vector2i(w, h)
	_administrative_water_cells = int(Dictionary(surface_result["stats"]).get("total_water_cells", -1))
...
func _dispatch_river_classify(w: int, h: int, groups_x: int, groups_y: int, atmosphere_type: int) -> void:
	...
	var river_biomes_data = PGPlanetData.build_river_biomes_gpu_buffer(planet_type, true)  # is_vegetation = true
...
func run_biome_phase(params: Dictionary, w: int, h: int) -> void:
	...
	var biomes_buffer_data = PGPlanetData.build_biomes_gpu_buffer(atmosphere_type)
	...
	print("  ✅ SSBO biomes créé: ", PGPlanetData.get_biome_gpu_count(atmosphere_type), " biomes (type=", atmosphere_type, ")")
...
func run_region_phase(params: Dictionary, w: int, h: int) -> void:
	...
	var land_mask_result: Dictionary = PGDepartmentNormalizer.build_surface_mask(
	...
	var hierarchy_targets := PGHierarchyBuilder.compute_physical_targets(
	...
	var normalization := PGDepartmentNormalizer.normalize(
...
func run_ocean_region_phase(params: Dictionary, w: int, h: int) -> void:
	...
	var physical_targets := PGHierarchyBuilder.compute_physical_targets(
...
func _run_water_to_color_phase(_params: Dictionary, _w: int, _h: int) -> void:
	# PGHydrologySolver is now the only authoritative component classifier and
...
func _run_final_map_shader(params: Dictionary, w: int, h: int) -> void:
	...
	var biomes_veg_data = PGPlanetData.build_biomes_gpu_buffer(atmosphere_type, true)  # is_vegetation = true
...
## Exporte toutes les cartes générées via PGPlanetExporter
## 
...
func cleanup() -> void:
	...
	# Relâcher les références propres à cet orchestrateur. PGGPUContext conserve
