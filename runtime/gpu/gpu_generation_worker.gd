extends RefCounted
class_name PGGPUGenerationWorker
...
static var _runner: PGGPUGenerationWorker = null
...
static func submit(job: Callable) -> bool:
	...
	if needs_start:
		_runner = PGGPUGenerationWorker.new()
		_thread = Thread.new()
		...
		if err != OK:
			...
			_runner = null
			_mutex.unlock()
			push_error("[PGGPUGenerationWorker] Unable to start background GPU worker: %s" % err)
			return false
	_jobs.append(job)
	_mutex.unlock()
	_semaphore.post()
