class_name TiledPhaseGpuRunner
extends RefCounted
...
## through GPUContext. No full-resolution global texture is ever allocated.

const SHADERS := {
	"terrain": "res://shader/compute/tiled_global/tiled_terrain.glsl",
	"erosion": "res://shader/compute/tiled_global/tiled_erosion.glsl",
	"climate": "res://shader/compute/tiled_global/tiled_climate.glsl",
	"hydrology": "res://shader/compute/tiled_global/tiled_hydrology.glsl",
	"classification": "res://shader/compute/tiled_global/tiled_classification.glsl",
}
...
var _macro_context: GlobalHydrologyContext
...
func _init(generation_params: Dictionary) -> void:
	...
	context = GPUContext.new(Vector2i.ONE)
...
func set_hydrology_context(macro: GlobalHydrologyContext) -> bool:
...
func generate_terrain(descriptor: Dictionary, cancel_token: GenerationCancelToken) -> Dictionary:
...
func erode_height(descriptor: Dictionary, input_height: PackedByteArray,
		iterations: int, cancel_token: GenerationCancelToken) -> Dictionary:
...
func generate_climate(descriptor: Dictionary, height_data: PackedByteArray,
		cancel_token: GenerationCancelToken) -> Dictionary:
	if cancel_token.is_cancelled() or not is_ready(): return {}
	var size: Vector2i = descriptor["sample_size"]
...
func generate_hydrology(descriptor: Dictionary, height_data: PackedByteArray,
		climate_data: PackedByteArray, cancel_token: GenerationCancelToken) -> Dictionary:
...
func generate_classification(descriptor: Dictionary, height_data: PackedByteArray,
		...
		flux_data: PackedByteArray, cancel_token: GenerationCancelToken) -> Dictionary:
	if cancel_token.is_cancelled() or not is_ready(): return {}
	var size: Vector2i = descriptor["sample_size"]
