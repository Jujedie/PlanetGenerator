class_name TiledGlobalSimulationPipeline
extends RefCounted
...
var generator: TiledGlobalGenerator
var store: PlanetTileStore
var reader: TileWindowReader
var gpu_runner: TiledPhaseGpuRunner
var cancel_token: GenerationCancelToken
...
func _init(generation_params: Dictionary, root_dir: String) -> void:
	...
	tile_size = int(params.get("tile_size", PlanetGridContract.DEFAULT_TILE_SIZE))
	output_root = root_dir
	generator = TiledGlobalGenerator.new(dimensions, tile_size)
	generator.hard_vram_budget_bytes = int(params.get("vram_budget_bytes", TiledGlobalGenerator.HARD_VRAM_BUDGET_BYTES))
	generator.preferred_vram_budget_bytes = mini(generator.hard_vram_budget_bytes, TiledGlobalGenerator.PREFERRED_VRAM_BUDGET_BYTES)
	cancel_token = generator.cancel_token
	store = PlanetTileStore.new(output_root)
	reader = TileWindowReader.new(store, dimensions, tile_size)
	dataset_fingerprint = TiledDatasetManifest.generation_fingerprint(params, dimensions, tile_size)
...
func generate() -> Dictionary:
	...
	gpu_runner = TiledPhaseGpuRunner.new(params)
	...
	var macro := GlobalHydrologyContext.build_from_height_tiles(
	...
	var manifest_path := TiledDatasetManifest.save(
	...
	if FileAccess.file_exists(state_path):
		...
		if file != null:
			...
			if parsed is Dictionary and str(parsed.get("fingerprint", "")) != dataset_fingerprint:
				...
				PlanetTileStore._remove_tree(output_root)
				DirAccess.make_dir_recursive_absolute(output_root)
...
func export_dataset(destination: String) -> bool:
	...
	return PlanetTileStore.copy_tree(output_root,destination)
...
func _terrain_tile(descriptor: Dictionary, token: GenerationCancelToken) -> Dictionary:
...
func _run_erosion() -> bool:
	...
	var max_chunk_halo:=mini(128,maxi((TiledGlobalGenerator.MAX_TILE_SAMPLE_EDGE-tile_size)/2,1))
...
func _copy_height_tile(descriptor: Dictionary, token: GenerationCancelToken) -> Dictionary:
	if token.is_cancelled():return {}
	var tile: Vector2i=descriptor["tile"]
	return {"height":reader.read_core("height_base",0,tile)}

func _erosion_tile(descriptor: Dictionary, token: GenerationCancelToken,
...
func _climate_tile(descriptor: Dictionary, token: GenerationCancelToken) -> Dictionary:
...
func _hydrology_tile(descriptor: Dictionary, token: GenerationCancelToken) -> Dictionary:
...
func _classification_tile(descriptor: Dictionary, token: GenerationCancelToken) -> Dictionary:
	var tile: Vector2i=descriptor["tile"]
...
func _remove_layer_files(layer: String)->void:
	PlanetTileStore.remove_layer(output_root,layer)
