...
# Textures Étape 4.6 - Hiérarchie Administrative (Département→Région→Pays→Continent)
# Terre : 3 niveaux hiérarchiques (région, pays, continent) + colored
# Mer : 3 niveaux hiérarchiques (région_mer, bassin, océan) + colored
# Temporaires partagés : map_temp, cost, cost_temp (réutilisés entre niveaux)
static var TextureID_Hierarchy : Array[String] = [
	"hier_terre_region_map", "hier_terre_pays_map", "hier_terre_continent_map",
	"hier_terre_region_colored", "hier_terre_pays_colored", "hier_terre_continent_colored",
	"hier_mer_region_map", "hier_mer_bassin_map", "hier_mer_ocean_map",
	"hier_mer_region_colored", "hier_mer_bassin_colored", "hier_mer_ocean_colored",
	"hier_map_temp", "hier_cost", "hier_cost_temp"
]
...
# === CRÉATION DES TEXTURES HIÉRARCHIE ADMINISTRATIVE (Étape 4.6) ===
func initialize_hierarchy_textures() -> void:
	"""
	Initialise les textures pour la hiérarchie administrative (étape 4.6).
	Crée 6 cartes R32UI (3 terre + 3 mer), 6 RGBA8 colorées, et 3 temporaires partagées.
	"""
	
	# Format R32UI pour IDs hiérarchiques (4 bytes par pixel)
	var format_r32ui := RDTextureFormat.new()
	format_r32ui.width = resolution.x
	format_r32ui.height = resolution.y
	format_r32ui.format = FORMAT_R32UI
	...
	# Format R32F pour coûts JFA (4 bytes par pixel)
	var format_r32f := RDTextureFormat.new()
	format_r32f.width = resolution.x
	format_r32f.height = resolution.y
	format_r32f.format = FORMAT_R32F
	...
	# Créer les 6 cartes R32UI (terre: région, pays, continent; mer: idem)
	var r32ui_maps = [
		"hier_terre_region_map", "hier_terre_pays_map", "hier_terre_continent_map",
		"hier_mer_region_map", "hier_mer_bassin_map", "hier_mer_ocean_map"
	]
	for tex_id in r32ui_maps:
		if not textures.has(tex_id):
			var data = PackedByteArray()
			data.resize(resolution.x * resolution.y * 4)
			for i in range(0, data.size(), 4):
	...
	# Créer hier_map_temp (R32UI partagé pour ping-pong)
	if not textures.has("hier_map_temp"):
		var data = PackedByteArray()
		data.resize(resolution.x * resolution.y * 4)
		for i in range(0, data.size(), 4):
			data.encode_u32(i, 0xFFFFFFFF)
		...
		var rid := rd.texture_create(format_r32ui, view, [data])
		if rid.is_valid():
			textures["hier_map_temp"] = rid
		else:
			push_error("❌ Échec création texture hier_map_temp")
	
	# Créer hier_cost et hier_cost_temp (R32F, init 1e30)
	for tex_id in ["hier_cost", "hier_cost_temp"]:
		if not textures.has(tex_id):
			var data = PackedByteArray()
			data.resize(resolution.x * resolution.y * 4)
			for i in range(0, data.size(), 4):
	...
	# Créer les 6 textures RGBA8 colorées
	var rgba8_maps = [
		"hier_terre_region_colored", "hier_terre_pays_colored", "hier_terre_continent_colored",
		"hier_mer_region_colored", "hier_mer_bassin_colored", "hier_mer_ocean_colored"
	]
	for tex_id in rgba8_maps:
		if not textures.has(tex_id):
			var data = PackedByteArray()
			data.resize(resolution.x * resolution.y * 4)
			data.fill(0)
	...
	print("✅ Textures hiérarchie créées (7x R32UI + 2x R32F + 6x RGBA8)")

## Réinitialise les textures temporaires de hiérarchie pour un nouveau niveau.
## Appelé avant chaque passe seed→growth→snap→cleanup pour remettre cost à 1e30.
func reset_hierarchy_temp_textures() -> void:
	# Réinitialiser hier_map_temp à 0xFFFFFFFF
	var data_r32ui = PackedByteArray()
	data_r32ui.resize(resolution.x * resolution.y * 4)
	for i in range(0, data_r32ui.size(), 4):
		data_r32ui.encode_u32(i, 0xFFFFFFFF)
	if textures.has("hier_map_temp") and textures["hier_map_temp"].is_valid():
		rd.texture_update(textures["hier_map_temp"], 0, data_r32ui)
	
	# Réinitialiser hier_cost et hier_cost_temp à 1e30
	var data_r32f = PackedByteArray()
	data_r32f.resize(resolution.x * resolution.y * 4)
	for i in range(0, data_r32f.size(), 4):
		data_r32f.encode_float(i, 1e30)
	for tex_id in ["hier_cost", "hier_cost_temp"]:
		if textures.has(tex_id) and textures[tex_id].is_valid():
			rd.texture_update(textures[tex_id], 0, data_r32f)
